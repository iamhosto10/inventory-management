  ...
  return (
    <div className="md:row-span-1 xl:row-span-2 bg-white col-span-1 shadow-md rounded-2xl flex flex-col justify-between">
      ...
      <div className="flex mb-6 items-center justify-around gap-4 px-5">
        <div className="rounded-full p-5 bg-green-200 border-green-300 border-[1px]">
          {primaryIcon}
        </div>
          ...
          {details.map((detail, index) => (
            <React.Fragment key={index}>
              <div className="flex items-center justify-between my-4">
                <span className="text-gray-500 w-[112px] ">{detail.title}</span>
                <span className="font-bold text-gray-800 text-right w-[50px]">
                  {detail.amount}
                </span>
